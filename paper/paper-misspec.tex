...
We define the threshold on these smoothed differences based on visual inspection of their histogram (0.03).
This results in an initial set of 9,394,361 variants.

From this large set of variants, we define two sets.
...
For the second set, we aim at maximizing the tagging of the initial 9,394,361 variants, i.e. $\sum_{i \in \text{all}} \max_{j \in \text{set}}{|R_{i,j}|}$ (inspired from the alternative sensitivity of \cite{agier2016systematic}). 
We design a greedy algorithm that first select all HapMap3 variants, then add one variant at a time, the one that maximizes the addition to this sum, until no variant can add more than 0.2.
