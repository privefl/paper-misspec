...
\author{Florian Priv\'e,$^{\text{1,}*}$ Julyan Arbel,$^{\text{2}}$ Timothy S. H. Mak,$^{\text{3}}$ and Bjarni J. Vilhj\'almsson$^{\text{1,4}}$}
...
\noindent$^{\text{\sf 4}}$Bioinformatics Research Centre, Aarhus University, Aarhus, 8000, Denmark. \\
...
First, when using imputed data, we multiply the correlation between variants $j$ and $k$ by $\sqrt{\text{INFO}_j \cdot \text{INFO}_k}$ (for $j \neq k$) since it should approximate the correlation from non-imputed data ([TODO: FIGURE]).
Second, we also define nearly independent LD blocks using the new optimal algorithm developed in \cite{prive2021optimal}.
...
Having a correlation matrix with independent blocks prevents the small errors in the algorithm (e.g.\ the Gibbs sampler in LDpred2) to propagate to the whole genome.
...
Finally, we have developed a new ``compact'' format for the SFBMs (sparse matrix on disk). Instead of using something similar to the ``compressed sparse column'' format which stores all $\{i,~x(i, j)\}$ for a given column $j$, we only store the first index $i_0$ and all the contiguous non-zero values $\{x(i_0, j),~x(i_0 + 1, j),~\dots\}$. This makes this format twice as efficient for both LDpred2 and lassosum2.
