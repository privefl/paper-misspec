...
Therefore, providing an accurate imputation INFO score is useful for two reasons. First, it allows for correcting for a reduced standard deviation when using imputed data in the QC step we propose, in order to better uncover problems with the summary statistics.
...
In the comparison, we use the QC proposed in \cite{prive2020ldpred2} first (which ends up filtering on MAF here, which we call ``qc1''). We then also filter out the two long-range LD regions of chromosome 6 and 8 for the OncoArray summary statistics and remove around 500 variants when filtering on differences of MAFs between summary statistics and the validation dataset (``qc2'').
...
Although results for both QC used are very similar, correcting using INFO scores slightly improves predictive performance when deriving polygenic scores based on iCOGS summary statistics (Figure \ref{fig:res_brca}). 
...
In figures \ref{fig:res_cad}-\ref{fig:res_t1d}, we then provide similar results as figure \ref{fig:res_brca} for other phenotypes.
Most changes introduced before have little to no impact on predictive performance, except for the second quality control performed for CAD when using LDpred2-auto (Figure \ref{fig:res_cad}).
...
Note that, in this study, we use GWAS summary statistics that include extended information (e.g.\ INFO scores and allele frequencies), yet most GWAS summary statistics do not provide such exhaustive information \cite[]{macarthur2021workshop}.
...
It would also be useful to investigate the impact of the QC and adjustments we propose here on other (non-PGS) methods, for which consistency and robustness are likely to be very important as well \cite[]{chen2021improved}.
...
\begin{table}[h]
	...
	\begin{tabular}{|l||c|c|c||c|c|}
		...
		\hline
	\end{tabular}
	\caption{Summary of external GWAS summary statistics used. PRCA summary statistics have many variants with a missing INFO score, which we discard. We also restrict to variants with an INFO score larger than 0.4.\label{tab:sumstats}}
\end{table}
