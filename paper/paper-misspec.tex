...
We then run C+T, LDpred2-inf, LDpred2(-grid), LDpred2-auto, lassosum, lassosum2, PRS-CS and SBayesR by using either the true per-variant GWAS sample sizes, the total sample size, or imputed sample sizes. 
...
However, correcting for imputation quality can dramatically improves predictive performance for LDpred2-auto. 
...
We try this for the two breast cancer summary statistics. We first compare the standard QC proposed in \cite{prive2020ldpred2} (which ends up filtering on MAF here, which we call ``qc1''). We then also filter out the two long-range LD regions of chromosome 6 and 8 for the OncoArray summary statistics and remove around 500 variants when filtering on differences of MAFs between summary statistics and the validation dataset (``qc2'').
...
\subsection{Larger sets of variants and other phenotypes}
...
We then show results with two new sets of variants (Methods) in figures \ref{fig:res_brca_all}-\ref{fig:res_t1d_all}.
[WHAT TO SAY ABOUT THIS? MAXTAG GOOD FOR SOME TRAITS, CLUMP NOT GREAT. PUT ONE FIGURE IN MAIN TEXT?]
