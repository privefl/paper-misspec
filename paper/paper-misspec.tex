...
Multiple studies have shown that LDpred2 and lassosum are consistently ranking best among methods for polygenic prediction \cite[]{mak2017polygenic,prive2020ldpred2,pain2021evaluation,kulm2021systematic}.
These two methods can be widely used because they require only summary statistics from genome-wide association studies (GWAS), as opposed to individual-level genotypes and phenotypes.
However, GWAS summary statistics come with different qualities, such as the imputation qualities of each variant reported. They also come with some heterogeneity in the methods used for performing the different GWAS and meta-analyses, as well as the information reported in the summary statistics.
LDpred2, as many other Bayesian polygenic methods \cite[]{ge2019polygenic,lloyd2019improved}, uses a Gibbs sampler, which is known to be sensitive to model misspecifications [TODO: @JULYAN, GOOD REFS FOR THAT?].
Previously, to make sure that input parameters passed to LDpred2 (and especially its ``auto'' version) were consistent with what LDpred2 was expecting, we proposed a quality control (QC) based on comparing standard deviations derived from GWAS summary statistics with the ones from a reference panel \cite[]{prive2020ldpred2}.

Here, we further investigate some of the possible misspecifications that come with the use of GWAS summary statistics. 
We aim at identifying and correcting multiple sources of misspecifications in the summary statistics in order to improve the predictive performance of polygenic scores.
We work on three different angles.
First, based on other information such as the imputation INFO scores and allele frequencies from the GWAS summary statistics, we refine the QC we proposed in \cite{prive2020ldpred2}.
...
Second, we investigate possible corrections to apply to the input parameters of polygenic methods, namely the LD (linkage disequilibrium) reference, the GWAS effect sizes, their standard errors and the corresponding sample sizes.
For example, we show that GWAS effect sizes computed from imputed dosages are larger than if true genotypes were available.
Third, we introduce small changes to LDpred2 to make it more robust to these misspecifications.
For further investigations, we also reimplement a new version of lassosum, called lassosum2, that uses the exact same input parameters as LDpred2, and test the QCs and corrections for lassosum2 as well.
We first perform our investigations in simulations, then we use published summary statistics while restricting to the widely-used HapMap3 variants, finally we investigate two alternative sets with more variants.
...
[SHOULD I USE ``INFO'' INSTEAD? BUT I NEED TO RERUN EVERYTHING BECAUSE I CAN'T JUST POSTPROCESS RESULTS. AND NOT BETTER FOR LDPRED2-LOW-H2 AND LDPRED2-AUTO-ROB]
...
Here we show that we can refine this quality control by deriving the second standard deviations directly from the reported allele frequencies in the GWAS summary statistics, and by correcting for the INFO scores of the GWAS (e.g.\ see figure \ref{fig:brca_onco_qc}).
...
Another information, the effective sample size per variant, is often missing from GWAS summary statistics. Even the total effective sample size is sometimes difficult to assess. Indeed, often only the total number of cases and controls are reported in the abstract of GWAS papers, where reported numbers possibly includes sizes before QC; therefore one has to dig into supplementary materials to find this information.
We also recall that if some studies of a meta-analysis have an imbalanced number of cases and controls, the global effective sample size should not be computed from the total numbers of cases and controls overall, but instead from the sum of each effective sample size.
...
Overall, these changes have provided large improvements of predictive performance in simulations with large misspecifications. However, they have almost no effect when using the published GWAS summary statistics, except for breast cancer and coronary artery disease summary statistics.
This is slightly disappointing, but more importantly it is also reassuring because it means that the summary statistics we use in this study are usually of good quality for follow-up analyses such as deriving polygenic scores.
