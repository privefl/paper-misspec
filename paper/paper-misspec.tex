...
Contrary to individual-level genotypes and phenotypes, summary statistics resulting from genome-wide association studies (GWAS) are widely available, and one can obtain very large sample sizes through meta-analysis \cite[]{yengo2022saturated}.
...
First, the total sample size used can be misestimated, which would result e.g.\ in a biased estimation of the SNP heritability. For example, the total effective sample size is overestimated when computing it from the total number of cases and controls from a meta-analysis of binary outcomes \cite[]{grotzinger2021pervasive}; using BOLT-LMM summary statistics can result in an increased effective sample size \cite[]{loh2018mixed,gazal2018functional}; using SAIGE on binary traits with a large prevalence can result in a reduced effective sample size \cite[]{zhou2018efficiently,mbatchou2021computationally}. 
...
As a QC step, one can e.g.\ remove SNPs with a sample size less than 0.67 times the 90th percentile of all sample sizes \cite[]{zheng2017ld}.
Third, many genetic variants are imputed, which can cause some bias in summary statistics.
Fourth, the quality of imputation of each variant, often used in a quality control (QC) step in statistical genetics analyses, can be severely misestimated (often overestimated) when computed for multi-ancestry individuals instead of a more homogeneous subset.
Fifth, there can be some errors in the summary statistics (e.g.\ allele inversions); QC is particularly important here.
...
Breast cancer summary statistics are interesting because they include results from two mega analyses \cite[]{michailidou2013large,michailidou2015genome,michailidou2017association}, which means there is some larger precision in the parameters reported in the GWAS summary statistics, such as the INFO scores and the sample sizes.
...
Anyway, we recommend to adopt these changes, i.e.\ performing the (refined) QC proposed here, defining LD blocks in the LD matrix, and using more regularization when needed (i.e.\ also testing smaller values for the heritability parameter in LDpred2-grid, and using the two new parameters in LDpred2-auto (Section \ref{sec:robust}). 
Note that LD blocks are widely used already by several methods (e.g.\ lassosum and PRS-CS) because they allow for processing smaller matrices at once \cite[]{mak2017polygenic,ge2019polygenic}; here we have shown that these blocks are also useful to make methods more robust.
PRS-CS is one of the most robust PGS methods out there; for example, it can use the (small) 1000 Genomes data as LD reference \cite[]{ge2019polygenic}, and can even use an European LD reference panel with multi-ancestry GWAS summary statistics \cite[]{wang2021global}.
We think this is made possible by the use of a (very) strong regularization in PRS-CS ($\phi^{-1} \psi_j^{-1} \ge 1$, which would approximately corresponds to using $s=0.5$ in lassosum, $\delta = 1$ in lassosum2, and $\texttt{shrink\_corr}=0.5$ in LDpred2-auto).
...
As a future direction, we are interested in using ancestry-matched LD matrices to improve polygenic prediction in all ancestries.
...
We sample 300,000 individuals to form a training set (e.g.\ to run the GWAS), 10,000 individuals to form a validation set (to tune hyper-parameters), and use the remaining 52,307 individuals to form a test set (to evaluate final predictive models).
