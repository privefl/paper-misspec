...
\cite{marchini2010genotype} showed that the IMPUTE INFO measure is highly concordant with the MACH measure $\hat{r}_j^2 = \frac{var(G_j)}{2 \hat{\theta} (1 - \hat{\theta_j})}$, where $\hat{\theta_j}$ is the estimated allele frequency of variant $j$.
...
\begin{figure}[h]
	...
	Correction ``info'' corresponds to using $\hat{\gamma}_j^{imp} \cdot \text{INFO}_j$  and $N \cdot \text{INFO}_j$. Correction ``in\_between'' corresponds to using $\hat{\gamma}_j^{imp} \cdot \text{INFO}_j$, $\text{se}(\hat{\gamma}_j)^{imp} \cdot \sqrt{\text{INFO}_j}$, and $N \cdot \text{INFO}_j$.}
	\label{fig:simu-info}
\end{figure}
