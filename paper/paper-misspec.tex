...
\begin{table}[h]
	...
	\begin{tabular}{|p{\dimexpr0.2\textwidth-2\tabcolsep-\arrayrulewidth\relax}|
		...
		\hline
		Misspecification & Possible consequence & Possible correction \\
		\hline
		\hline
		...
		\hline
		Using imputed dosages for LD computation & Overestimated LD [TODO: ALSO VERIFY WITH 1000G] & See correction in section \ref{sec:LD} \\
		\hline 
		...
		\hline
		Error in summary statistics (e.g.\ allele inversions) & Strong violation of model assumptions, which can result in e.g.\ identifying false-positives in fine-mapping \cite[]{susie} & Appropriate quality control \\
		\hline
		Any & Violation of model assumptions and bad model convergence & Using more regularization \\
		\hline
	\end{tabular}
	...
	$^{(2)}$ E.g.\ removeing SNPs with an effective sample size less than 0.67 times the 90th percentile of sample size \cite[]{zheng2017ld}.
	\label{tab:overview}}
\end{table}
