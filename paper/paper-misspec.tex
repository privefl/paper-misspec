...
GWAS summary statistics have been extensively used to derive polygenic scores (PGS), perform fine-mapping, and estimate a range of key genetic architecture parameters.
...
Fourth, the imputation quality of each variant (e.g.\ INFO scores), often used in a QC step in statistical genetics analyses, can be severely misestimated (often overestimated) when computed for multi-ancestry individuals instead of a more homogeneous subset.
...
As for the correction using the INFO scores, we use the first correction, ``sqrt\_info'', which is simple because it is equivalent to post-processing PGS effects by multiplying them by $\sqrt{\text{INFO}}$.
Although results for both QC used are very similar, correcting using the INFO scores slightly improves prediction when deriving polygenic scores based on iCOGS summary statistics (Figure \ref{fig:res_brca}). 
...
Also it would be useful to investigate the impact of the QC and adjustments we propose here on other (non-PGS) methods, where robustness is likely to be very important as well.
