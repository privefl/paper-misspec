...
\title{Identifying and correcting multiple sources of misspecification\\in GWAS summary statistics for prediction\\---~OR~---\\Reducing misspecifications and improving tagging\\for better polygenic scores}
...
[SHOULD ADD THE LDAK-ALPHA IN THIS WORK? MAYBE LATER -> IN A THIRD VERSION]
...
[WHY CHOOSED TO FOCUS ON LDPRED AND LASSOSUM -> COPY PREVIOUS INTRO + MORE CONTROL ON THE IMPLEMENTATION]
...
We then run C+T, LDpred2-inf, LDpred2(-grid), LDpred2-auto, lassosum, lassosum2 (Methods), PRS-CS and SBayesR \cite[]{prive2019making,prive2020ldpred2,mak2017polygenic,ge2019polygenic,lloyd2019improved} by using either the true per-variant GWAS sample sizes, the total sample size, or imputed sample sizes. 
...
\begin{figure}[h]
	...
	\caption{Results for the simulations with sample size misspecification. The GWAS sample size is ``true'' when providing the true per-variant sample size, ``max'' when providing instead the maximum sample size as a unique value to be used for all variants,  ``imputed'' (Methods), or ``any'' when the method does not use this information (the case for C+T). Red bars correspond to using the LD with independent blocks (Methods).}
	\label{fig:simu-misN}
\end{figure}
...
\cite{marchini2010genotype} showed that the IMPUTE INFO measure is highly concordant with the MACH measure $\hat{r}_j^2 = \frac{var(G_j)}{2 \hat{\theta}_j (1 - \hat{\theta}_j)}$, where $\hat{\theta}_j$ is the estimated allele frequency of variant $j$.
Therefore, when using the expected genotypes from imputation (dosages), their standard deviations are often lower than the expected value under Hardy-Weinberg equilibrium, because $\text{INFO}_j \approx \frac{var(G_j)}{2 \hat{\theta}_j (1 - \hat{\theta}_j)}$.
In simulations (cf.\ Methods section ``Data for simulations''), we verify that $\text{sd}(G_j)^{true} \approx \text{sd}(G_j)^{imp} / \sqrt{\text{INFO}_j}$ (Figure \ref{fig:compare-sd}).
...
QC plots comparing standard deviations usually look good (after the INFO score correction), except for coronary artery disease (CAD) summary statistics (Figures \ref{fig:brca_icogs_qc}-\ref{fig:t1d_illu_qc}).
...
Here we have investigated misspecifications in GWAS summary statistics, including when they are derived from imputed data, and when the information on sample size per variant is different for all variants because of meta-analysis.
Using LDpred2 models \cite[]{prive2020ldpred2} and our reimplementation of the lassosum model \cite[]{mak2017polygenic} for deriving polygenic scores, we have introduced some changes to possibly make these models more robust to these misspecifications.
Previously, we proposed a quality control based on comparing standard deviations derived from GWAS summary statistics with the ones from a reference panel \cite[]{prive2020ldpred2}.
Here we show that we can refine this quality control and almost get perfect matches by deriving standard deviations from reported allele frequencies in the summary statistics (instead of from the reference panel, which can now be used to compare allele frequencies), and by also correcting for the INFO score (e.g.\ see figure \ref{fig:brca_onco_qc}).
Using this refined QC, we were able to identify a potential issue with how principal components were derived in a set of breast cancer summary statistics. Fortunately this has practically no effect on the predictive performance of the derived polygenic scores.

Note that, in this study, we use summary statistics for which this information (INFO scores and allele frequencies) is available, but this is far from being the case for all GWAS summary statistics. 
We acknowledge that, in the case of a meta-analysis from multiple studies, providing a single INFO score may not be possible; would a weighted averaged INFO score work?
...
Finally, another information, the effect sample size per variant, is often missing from GWAS summary statistics. 
We recall that if some studies of a meta-analysis have an imbalanced number of cases and controls, the global effective sample size should not be computed from the total numbers of cases and controls overall, but instead from the sum of each effective sample size.
...
Again, we note that an overestimated sample size can be detected from the QC plot we propose (when the slope is less than 1) for case-control studies using logistic regression; otherwise the standard deviation of the phenotype is also needed (Equation \ref{eq:approx-sd-lin}).
...
[TALK ABOUT SUM(N\_EFF) VS N\_EFF(SUM)?]

[BOTH DISAPOINTING BUT REASSURING]
...
We use external published GWAS summary statistics listed in table \ref{tab:sumstats} to derive polygenic scores.
...
\begin{table}[h]
	...
	\begin{tabular}{|l||c|c|c||c|c|}
		...
		\hline
		Breast cancer (BRCA) [iCOGS] & \cite{michailidou2017association} & ~~87036.6 & 11,792,542 & 1,051,242 & 0.841 \\
		Breast cancer (BRCA) [OncoArray] & \cite{michailidou2017association} & 104441.6 & 11,792,542 & 1,054,233 & 0.968 \\
		Type 1 diabetes (T1D) [Affymetrix] & \cite{censin2017childhood} & ~~~~5515.6  & ~~8,996,866 & ~~~934,712 & 0.885 \\	
		Type 1 diabetes (T1D) [Illumina] & \cite{censin2017childhood} & ~~~~7982.0  & ~~8,996,866 & ~~~949,334 & 0.942 \\
		Prostate cancer (PRCA) & \cite{schumacher2018association} & 135316.1 & 20,370,946 & ~~~818,400 & 0.969 \\
		Depression (MDD) [without UKBB] & \cite{wray2018genome} & 110463.7 & ~~9,874,289 & 1,049,455 & 0.968 \\
		Coronary artery disease (CAD) & \cite{nikpay2015comprehensive} & 129014.3 & ~~9,455,778 & 1,052,200 & 0.982 \\
		\hline
	\end{tabular}
...
From this large set of variants, we define the two sets.
...
For the second set, we aim at maximizing the tagging of all the initial 9,394,361 variants, i.e. $\sum_{i \in \text{all}} \max_{j \in \text{set}}{|R_{i,j}|}$ (inspired from the alternative sensitivity of \cite{agier2016systematic}). 
