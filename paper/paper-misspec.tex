...
{\color{red}Note that a new parameter \texttt{max\_r2} has been added to this method since, which controls the maximum single squared correlation allowed outside blocks, which offers an extra guarantee that the splitting is good and makes the function much faster by discarding lots of possible splits.
...
{\color{red}Here MI consists in forming multiple (e.g.\ 20) datasets of hard-called genotypes by sampling them according to the imputation probabilities stored in the BGEN files, performing a GWAS on each of them, and pooling the GWAS estimates.}
