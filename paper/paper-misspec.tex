...
\title{Identifying and correcting multiple sources of misspecification\\in GWAS summary statistics}
...
\subsection*{GWAS using expected genotypes from imputation}
...
Note that we recompute INFO scores for the subset of 362,307 European individuals used in this paper as they sometimes differ from the ones reported by the UK Biobank (Figure \ref{fig:compare-info}).
Instead of using dosages to perform the GWAS, it has been argued that using multiple imputation (MI) would be more appropriate \cite[]{Palmer2016}.
...
We run GWAS on the dosage dataset and use these summary statistics to run LDpred2 and lassosum2 with either no correction of the summary statistics, or with one of the two described above.
...
Correction ``sqrt\_info'', which is equivalent to postprocessing PGS effects by $\sqrt{\text{INFO}}$ slighly improves predictive performance for all methods, but correction ``info'' (using $\hat{\beta}_j^{imp} \cdot \text{INFO}_j$  and $N \cdot \text{INFO}_j$) provides much better predictive performance for LDpred2-auto, with a mean squared correlation of 15.4\% compared to 12.9\% with no correction and 13.3\% with correction ``sqrt\_info'' (Figure \ref{fig:simu-info}).
...
Breast cancer summary statistics are interesting because they come from two mega analyses [TODO: VERIF] \cite[]{michailidou2013large,michailidou2017association}, which means there is some larger precision in the parameters reported, such as the INFO scores and the sample sizes.
INFO scores for the OncoArray summary statistics (after having restricted to HapMap3 variants) are globally very good (Figure \ref{fig:hist-info-brca}), but the ones from iCOGS are a bit worse (Figure \ref{fig:hist-info-brca2}), probably because they are older.
