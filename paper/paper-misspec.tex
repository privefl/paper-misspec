...
\title{Identifying and correcting for misspecifications\\in GWAS summary statistics for polygenic scores}
...
\abstract{
	...
	[TODO: LIST SOME OF THEM (AND POSSIBLE CONSEQUENCES?)]
}
...
Therefore, they have been extensively used to e.g.\ derive polygenic scores (PGS), perform fine-mapping, and estimate key genetic architecture parameters.
However, GWAS summary statistics come with uneven quality, such as the imputation accuracy of each variant reported. 
There is also heterogeneity in the information made available in these summary statistics, e.g.\ per-variant imputation INFO scores, sample sizes and allele frequencies are often missing.
...
First, the total sample size used can be misestimated, which would result e.g.\ in a biased estimation of the SNP heritability. For example, the total effective sample size is overestimated when computed from the total number of cases and controls from a meta-analysis of binary outcomes \cite[]{grotzinger2021pervasive}; using BOLT-LMM summary statistics can result in an increased effective sample size \cite[]{loh2018mixed,gazal2018functional}; using SAIGE on binary traits with a large prevalence can result in a reduced effective sample size \cite[]{zhou2018efficiently,mbatchou2021computationally}. 
Second, per-variant sample sizes can be very different when meta-analyzing GWAS summary statistics from multiple cohorts with different sets of variants \cite[]{wang2021global}.
Per-variant sample sizes should be used instead of the overall total sample size to avoid any bias resulting from overestimating the sample size.
Moreover, using very different per-variant sample sizes could be problematic for models implicitly assuming that summary statistics have all been derived from the same individuals \cite[]{zhu2017bayesian,zhou2021fast}.
...
Fourth, the quality of imputation of each variant, often used in a QC step in statistical genetics analyses, can be severely misestimated (often overestimated) when computed for multi-ancestry individuals instead of a more homogeneous subset.
...
\begin{table}[!htb]
	...
	\begin{tabular}{|p{\dimexpr0.21\textwidth-2\tabcolsep-\arrayrulewidth\relax}|
		...
		\hline
		Differences in per-variant sample size & Possible violation of model assumptions resulting in poor model convergence & Imputation using equations \ref{eq:imputeN} and \ref{eq:imputeNeff}, and filtering$^{(2)}$   \\
		\hline
		...
		\hline 
		Misestimation of imputation INFO scores & Misuse when filtering or adjusting input parameters based on these & Recomputing for homogeneous subset \\
		\hline
		...
		\hline
		Rounding of summary statistics (effect sizes and SEs) & Add unnecessary noise to the data & None, but this has almost no impact on the predictive performance (Figure \ref{fig:simu-rounded}) \\
		\hline 
...
Here, we show e.g.\ that standard deviations of imputed genotypes (allele dosages) are lower than the expected values under Hardy--Weinberg equilibrium.
Second, we investigate possible adjustments to apply to the input parameters of methods using summary statistics (focusing on PGS methods here), namely the reference LD (linkage disequilibrium) matrix, the GWAS effect sizes, their standard errors and the corresponding sample sizes.
...
We design simulations where variants have different GWAS sample sizes, which is often the case when meta-analyzing GWAS summary statistics from multiple cohorts with different sets of variants \cite[]{wang2021global}.
...
Note that we initially included PRS-CS and SBayesR in our comparison \cite[]{ge2019polygenic,lloyd2019improved}. However, results for SBayesR always diverged (stopped with an error), and the overlap with the LD reference provided for PRS-CS was too small.
...
To overcome this limitation, we introduce two new parameters to make LDpred2-auto more robust, referred to as ``LDpred2-auto-rob'' here (Methods).
...
Note that we recompute INFO scores for the subset of 362,307 European individuals used in this paper as they can differ substantially from the ones reported by the UK Biobank for the whole data (Figures \ref{fig:compare-info} and \ref{fig:compare-maf-info}).
...
This is the second correction of summary statistics we implement in the simulations below, along with $n_j = N \cdot \text{INFO}_j$ where $n_j$ and $N$ are the per-variant and total sample sizes. 
...
Averaged over 10 simulations, when using an ``alternative'' LD reference panel instead of a well-matched one, squared correlations between the polygenic scores and the simulated phenotypes drop from 0.174 to 0.167 for lassosum2, from 0.168 to 0.147 for LDpred2(-grid), from 0.174 to 0.169 for LDpred2-low-h2, from 0.143 to 0.139 for LDpred2-auto, from 0.162 to 0.146 for LDpred2-auto-rob, and from 0.144 to 0.140 for LDpred2-inf (Figure \ref{fig:simu-altpop}).
Defining LD blocks in these two LD matrices always helps, but do not improve performance by much when using the alternative LD.

\subsection{Application to breast cancer summary statistics}

Breast cancer summary statistics are interesting because they include results from two mega analyses \cite[]{michailidou2013large,michailidou2015genome,michailidou2017association}, which means that parameters reported in the GWAS summary statistics, such as the INFO scores and the sample sizes, are estimated with high precision.
...
For both summary statistics, we first compare the standard deviations (of genotypes) inferred from the reported allele frequencies (i.e.\ $\sqrt{2 f (1 - f)}$ where $f$ is the allele frequency, and denoted as sd\_af) versus the ones inferred from the GWAS summary statistics (Equation \eqref{eq:approx-sd-log}, and denoted as sd\_ss).
...
We hypothesize that this is due to using (as covariates in the GWAS) principal components (PCs)  that capture LD structure instead of population structure \cite[]{prive2020efficient}.
...
Second, using one of the proposed corrections based on the INFO scores may lead to an improved prediction when deriving polygenic scores.
...
We show previous results as figure \ref{fig:res_brca} for other phenotypes in figures \ref{fig:res_cad}-\ref{fig:res_t1d}; most changes introduced before have little to no impact on the predictions, except for the second quality control performed for CAD when using LDpred2-auto (Figure \ref{fig:res_cad}).
...
Here we show that we can refine this QC by deriving the latter directly from the reported allele frequencies in the GWAS summary statistics, and by correcting the former using the GWAS imputation INFO scores (e.g.\ see figure \ref{fig:brca_onco_qc}).
...
We do perform this additional QC as part of ``qc2'' here. 
...
Note that, in this study, we use summary statistics that include extended information (e.g.\ INFO scores and allele frequencies), yet most GWAS summary statistics do not provide such exhaustive information \cite[]{macarthur2021workshop}.
...
Nevertheless, we recommend to adopt these changes, i.e.\ performing the (refined) QC proposed here, defining LD blocks in the LD matrix, and using more regularization when needed (i.e.\ also testing smaller values for the heritability parameter in LDpred2-grid, and using the two new parameters in LDpred2-auto (Section \ref{sec:robust}). 
Note that LD blocks are already widely used by several methods (e.g.\ lassosum and PRS-CS) because they allow for processing smaller matrices at once \cite[]{mak2017polygenic,ge2019polygenic}; here we have shown that these blocks are also useful to make methods more robust.
...
Using enough regularization is good for robustness, but note that using too much regularization can also damage predictive performance, and should not be a replacement for a proper QC and choice of ancestry-matched LD [TODO: A BIT AGGRESSIVE?].
As a future research direction, we are interested in using ancestry-matched LD matrices to improve polygenic prediction in all ancestries.
Also it would be useful to investigate the impact of the QC and adjustments we propose here on other (non-PGS) methods, such as the ones for inference, where robustness is likely to be very important as well.
...
We have recomputed the allele frequencies and imputation INFO scores within the European subset used here and across all variants in the UK Biobank, and made them available at \url{https://doi.org/10.6084/m9.figshare.16635388.v1}.
