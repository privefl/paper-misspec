...
Therefore, when using the expected genotypes from imputation (dosages), their standard deviations are usually lower than the expected value under Hardy-Weinberg equilibrium because $\text{INFO}_j \approx \frac{var(G_j)}{2 \hat{\theta} (1 - \hat{\theta})}$.
In simulations (cf.\ Methods section ``Data for simulations''), we show that $\text{sd}(G_j)^{true} \approx \text{sd}(G_j)^{imp} / \sqrt{\text{INFO}_j}$, $\text{se}(\hat{\gamma}_j)^{true} \approx \text{se}(\hat{\gamma}_j)^{imp} \cdot \sqrt{\text{INFO}_j}$, and $\hat{\gamma}_j^{true} \approx \hat{\gamma}_j^{imp} \cdot \sqrt{\text{INFO}_j}$ (Figures \ref{fig:compare-sd}, \ref{fig:compare-beta}, and \ref{fig:compare-beta-se}).
...
In simulations, we show that $Z_j^{MI} \approx Z_j^{imp} \cdot \text{INFO}_j$ and $\hat{\gamma}_j^{MI} \approx \hat{\gamma}_j^{imp} \cdot \text{INFO}_j$ (Figure \ref{fig:compare-mi}).
...
\begin{figure}[h]
	...
	\caption{Results for the simulations using dosage data. Correction ``sqrt\_info'' corresponds to using $\hat{\gamma}_j^{imp} \cdot \sqrt{\text{INFO}_j}$ and $\text{se}(\hat{\gamma}_j)^{imp} \cdot \sqrt{\text{INFO}_j}$.
	Correction ``info'' corresponds to using $\hat{\gamma}_j^{imp} \cdot \text{INFO}_j$  and $N \cdot \text{INFO}_j$.}
	\label{fig:simu-info}
\end{figure}
...
As for the correction for the INFO scores, we use $\hat{\gamma}_j^{imp} \cdot \text{INFO}_j$ and $N \cdot \text{INFO}_j$ as we have shown to perform well in simulations.
