...
Previously, to make sure that the input parameters used in LDpred2 were consistent with its modeling assumptions, we proposed a quality control (QC) based on comparing standard deviations (of genotypes) inferred from GWAS summary statistics with the ones computed from a reference panel \cite[]{prive2020ldpred2}.
...
For example, we show that standard deviations of imputed genotypes (allele dosages) are lower than the expected values under Hardy-Weinberg equilibrium.
...
Second, we reimplement and use a new version of lassosum, called lassosum2, that uses the exact same input parameters as LDpred2, which makes it easy for us to test the different QCs and corrections presented here.
...
\begin{table}[h]
	...
	$^{(1)}$ Examples: using the global number of cases and controls in a meta-analysis of binary traits, a larger effective sample size when using BOLT-LMM summary statistics \cite[]{loh2018mixed}, and a reduced effective sample size when using SAIGE on binary traits with low case-control imbalance \cite[]{zhou2018efficiently,mbatchou2021computationally}. 
...
We design simulations where variants have different GWAS sample sizes, which is often the case when meta-analyzing GWAS from multiple cohorts without the same genome coverage \cite[]{wang2021global}.
...
[TODO: UPDATE AND REMOVE NEW SETS]
...
\end{equation}
In practice, we also bound this estimate to be between $0.5 \cdot N$ and $1.1 \cdot N$, where $N$ is the global (effective) sample size.
