...
\begin{figure}[p]
	...
	\end{subfigure}
	\caption{Multiple results (squared correlation $r^2$ between polygenic score and phenotype) over a grid of parameters ($\lambda$ and $\delta$) for lassosum2 in one of the simulations with misspecified GWAS sample sizes ($n_j$). The top subfigure is using the maximum of $n_j$'s, whereas the bottom subfigure is using the imputed $n_j$'s.  Missing points represent models that were detected as divergent. [TODO: USE GREEK LETTERS + FACETS FOR COMMON LEGEND SCALE]}
...
\begin{figure}[p]
	...
	\end{subfigure}
	\caption{Multiple results (squared correlation $r^2$ between polygenic score and phenotype) over a grid of parameters (the SNP heritability $h^2$ and the proportion of causal variants $p$) for LDpred2(-grid) in one of the simulations with misspecified GWAS sample sizes ($n_j$). The top subfigure is using the maximum of $n_j$'s, whereas the bottom subfigure is using the imputed $n_j$'s. Note that the small $h^2$ values (0.002, 0.02, 0.06) were added and are part of the method we call ``LDpred2-low-h2'' in the main text. Missing points represent models that were detected as divergent.}
...
\begin{figure}[p]
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{ldpred2-mis-maxN-corr2}
	\end{subfigure}\vspace{2em}
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{ldpred2-mis-imputeN-corr2}
	\end{subfigure}
	\caption{Same results as figure \ref{fig:ldpred2-misN}, but using the LD with independent blocks.}
	\label{fig:ldpred2-misN-corr2}
\end{figure}
