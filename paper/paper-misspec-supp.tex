...
\begin{figure}[p]
	...
	\caption{Results for the simulations with summary statistics with LD matrices based on two different populations, using a heritability of 4\% (instead of 20\%). One comes from the same ancestry used for computing the GWAS summary statistics (North-West Europe), while the other one comes from South Europe (alternative LD reference). Reported 95\% confidence intervals are computed from 10,000 non-parametric bootstrap replicates of the mean. Red bars correspond to using the LD with independent blocks (Methods), which is a requirement for PRS-CS.}
