...
\begin{figure}[p]
	...
	\caption{Results for the simulations using dosage data. The GWAS sample size is ``true" when providing the true per-variant sample size, ``max'' when providing instead the maximum sample size as a unique value to be used for all variants,  ``imputed'' (see Methods in main text), or ``any'' when the method does not use this information (the case for C+T).}
	\label{fig:simu-info}
\end{figure}
