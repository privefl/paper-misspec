...
all_final_grp <- furrr::future_map_dfr(1:22, function(chr) {
  ...
  corr0 <- readRDS(paste0("data/corr-large/clump_adj_chr", chr, ".rds"))
  dim(corr0)
  ...
  }, file = paste0("tmp-data/split_corr_clump_chr", chr, ".rds"))
  ...
  corr0T %>%
    Matrix::drop0() %>%
    as("symmetricMatrix") %>%
    saveRDS(paste0("data/corr-large/clump_adj_with_blocks_chr", chr, ".rds"))
...
sum(file.size(paste0("data/corr-large/clump_adj_with_blocks_chr", 1:22, ".rds"))) /
  sum(file.size(paste0("data/corr-large/clump_adj_chr", 1:22, ".rds")))
# 60.4%
