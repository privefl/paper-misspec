...
#### Get 504 "Japanese" individuals from the UKBB + 1000G ####
...
ind <- head(order(ldist_to_JPN), 504 - 104)
print_table(df_UKBB$country[ind])
# Japan: 241 || China: 100 || South Korea: 26 || NA: 9 || North Korea: 6 || Hong Kong: 5 || Brazil: 5


#### Read in + merge with the 104 from 1000G ####
...
snp_readBGEN(
  ...
  backingfile = "tmp-data/JPN_HM3",
  ind_row     = c(rep(1, 104), match(df_UKBB$eid[ind], sample$ID_2)),
  ncores      = nb_cores()
)

obj.bigsnp <- snp_attach("tmp-data/JPN_HM3.rds")
G <- obj.bigsnp$genotypes
obj.1000G <- snp_attach("data/1000G_HM3.rds")
ind_JPN_1000G <- which(obj.1000G$fam$Population == "JPT")
G[1:104, ] <- obj.1000G$genotypes$as.FBM()[ind_JPN_1000G, ]

af_1000G <- big_scale()(G, ind.row = 1:104, ncores = nb_cores())$center
af_UKBB  <- big_scale()(G, ind.row = 105:nrow(G), ncores = nb_cores())$center
...
bigassertr::assert_dir("data/corr_JPN")
bigassertr::assert_dir("data/corr_JPN/ukbb_for_prscs")
...
all_final_grp <- furrr::future_map_dfr(1:22, function(chr) {
  ...
  }, file = paste0("data/corr_JPN/chr", chr, ".rds"))
  ...
  }, file = paste0("data/corr_JPN/adj_with_blocks_chr", chr, ".rds"))


  # LD for PRS-CS (based on the same data and LD blocks)
  hdf5_file <- paste0("data/corr_JPN/ukbb_for_prscs/ldblk_ukbb_chr", chr, ".hdf5")
  runonce::skip_run_if({
...
# saveRDS(all_final_grp, "data/corr_JPN/all_final_grp.rds")

sum(file.size(paste0("data/corr_JPN/adj_with_blocks_chr", 1:22, ".rds"))) /
  sum(file.size(paste0("data/corr_JPN/chr", 1:22, ".rds")))
# 57.0%
...
bigsnpr:::write.table2(snpinfo, "data/corr_JPN/ukbb_for_prscs/for_prscs.bim")
...
bigreadr::fwrite2(snpinfo2, "data/corr_JPN/ukbb_for_prscs/snpinfo_ukbb_hm3", sep = "\t")
