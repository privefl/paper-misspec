...
bigassertr::assert_dir("results-maxtag")
...
plan(batchtools_slurm(resources = list(
  t = "12:00:00", c = NCORES + 2, mem = "150g",
...
furrr::future_pwalk(files, function(pheno, qc, name_corr) {
  ...
  gwas_file <- paste0("data/sumstats-maxtag/", pheno, "_", qc, ".rds")
  sumstats <- readRDS(gwas_file)
  basename <- file.path("results-maxtag", sub("\\.rds$", "", basename(gwas_file)))
  if (grepl("blocks", name_corr, fixed = TRUE))
    basename <- paste0(basename, "_adj_with_blocks")

  library(bigsnpr)
  CHR <- as.integer(snp_attach("data/UKBB_maxtag_val.rds")$map$chromosome)
