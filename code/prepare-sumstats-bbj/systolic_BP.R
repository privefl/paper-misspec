...
zip <- runonce::download_file(
  "https://pheweb.jp/download/SBP",
  dir = "tmp-data", fname = "sumstats_sbp_BBJ.zip")
unzip(zip, exdir = "tmp-data", overwrite = FALSE)

sumstats <- bigreadr::fread2(
  "tmp-data/hum0197.v3.BBJ.SBP.v1/GWASsummary_SBP_Japanese_SakaueKanai2020.auto.txt.gz",
...
# 7,444,410 variants to be matched.
# 0 ambiguous SNPs have been removed.
# 869,991 variants have been matched; 0 were flipped and 869,991 were reversed.

hist(info_snp$info)
mean(info_snp$info)  # 98.6%

info_snp2 <- info_snp %>%
  mutate(n_eff = 145505 * power,
...
saveRDS(filter(info_snp2, abs(sd_af - sd_ss2) < 0.03),
        "data/sumstats_BBJ/systolic_bp.rds")
