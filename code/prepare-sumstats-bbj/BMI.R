...
zip <- runonce::download_file(
  "https://pheweb.jp/download/BMI",
  dir = "tmp-data", fname = "sumstats_bmi_BBJ.zip")
unzip(zip, exdir = "tmp-data", overwrite = FALSE)

sumstats <- bigreadr::fread2(
  "tmp-data/hum0197.v3.BBJ.BMI.v1/GWASsummary_BMI_Japanese_SakaueKanai2020.auto.txt.gz",
...
# 7,444,325 variants to be matched.
# 0 ambiguous SNPs have been removed.
# 870,005 variants have been matched; 0 were flipped and 870,005 were reversed.

hist(info_snp$info)
mean(info_snp$info)  # 98.6%

info_snp2 <- info_snp %>%
  mutate(n_eff = 163835 * power,
