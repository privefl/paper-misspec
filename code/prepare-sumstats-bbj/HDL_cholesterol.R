...
zip <- runonce::download_file(
  "https://pheweb.jp/download/HDLC",
  dir = "tmp-data", fname = "sumstats_hdlc_BBJ.zip")
unzip(zip, exdir = "tmp-data", overwrite = FALSE)

sumstats <- bigreadr::fread2(
  "tmp-data/hum0197.v3.BBJ.HDLC.v1/GWASsummary_HDLC_Japanese_SakaueKanai2020.auto.txt.gz",
...
# 7,443,885 variants to be matched.
# 0 ambiguous SNPs have been removed.
# 870,022 variants have been matched; 0 were flipped and 870,022 were reversed.

hist(info_snp$info)
mean(info_snp$info)  # 98.6%

info_snp2 <- info_snp %>%
  mutate(n_eff = 74970 * power,
...
saveRDS(filter(info_snp2, abs(sd_af - sd_ss2) < 0.03),
        "data/sumstats_BBJ/hdl_cholesterol.rds")
