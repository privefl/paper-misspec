...
all_final_grp <- furrr::future_map_dfr(1:22, function(chr) {
  ...
  corr0 <- runonce::save_run({
    corr0 <- snp_cor(G, ind.row = ind_EAS, ind.col = ind.chr, infos.pos = POS2[ind.chr],
  ...
  }, file = paste0("tmp-data/split_corr_1000G_EAS_chr", chr, ".rds"))
  ...
  corr0T %>%
    Matrix::drop0() %>%
    as("symmetricMatrix") %>%
    saveRDS(paste0("data/corr_1000G_EAS/adj_with_blocks_chr", chr, ".rds"))
