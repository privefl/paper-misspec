...
ind_EAS <- which(obj.1000G$fam$`Super Population` == "EAS")
...
bigassertr::assert_dir("data/corr_1000G_EAS")
bigassertr::assert_dir("data/corr_1000G_EAS/1kg_for_prscs")

all_final_grp <- furrr::future_map_dfr(1:22, function(chr) {

  ind.chr <- which(CHR == chr)
  MAF <- snp_MAF(G, ind.row = ind_EAS, ind.col = ind.chr, ncores = nb_cores())
  keep <- (MAF > 0.02)
  ...
  corr0 <- runonce::save_run({
    ...
    snp_cor(G, ind.row = ind_EAS, ind.col = ind.chr2, infos.pos = POS2[ind.chr2],
            size = 3 / 1000, ncores = nb_cores())
  }, file = paste0("data/corr_1000G_EAS/chr", chr, ".rds"))
  ...
  }, file = paste0("data/corr_1000G_EAS/adj_with_blocks_chr", chr, ".rds"))


  # LD for PRS-CS (based on the same data and LD blocks)
  hdf5_file <- paste0("data/corr_1000G_EAS/1kg_for_prscs/ldblk_1kg_chr", chr, ".hdf5")
  runonce::skip_run_if({
    ind_block <- split(ind.chr2, best_grp)
    bigparallelr::set_blas_ncores(nb_cores())
    for (ic in seq_along(ind_block)) {
      ind <- ind_block[[ic]]
      ld <- big_cor(G, ind.row = ind_EAS, ind.col = ind)[]
...
# saveRDS(all_final_grp, "data/corr_1000G_EAS/all_final_grp.rds")

sum(file.size(paste0("data/corr_1000G_EAS/adj_with_blocks_chr", 1:22, ".rds"))) /
  sum(file.size(paste0("data/corr_1000G_EAS/chr", 1:22, ".rds")))
# 44.2%
...
bigsnpr:::write.table2(snpinfo, "data/corr_1000G_EAS/1kg_for_prscs/for_prscs.bim")
...
bigreadr::fwrite2(snpinfo2, "data/corr_1000G_EAS/1kg_for_prscs/snpinfo_1kg_hm3", sep = "\t")
