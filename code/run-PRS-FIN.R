...
library(dplyr)
grid <- tibble(pheno = c("brca", "prca", "cad", "t2d", "t1d")) %>%
  tidyr::expand_grid(name_corr = names(ALL_CORR)) %>%
  filter(!file.exists(paste0("results/sumstats_FIN/", pheno, "_", name_corr,
...
bigassertr::assert_dir("results/sumstats_FIN")
...
furrr::future_pwalk(grid, function(pheno, qc, name_corr) {
  ...
  basename <- paste0("results/sumstats_FIN/", pheno, "_", name_corr)
...
library(dplyr)
ALL_CORR <- c(
  corr_UK    = "data/corr/ukbb_for_prscs/ldblk_ukbb_chr1.hdf5",
  corr_FIN   = "data/corr_FIN/ukbb_for_prscs/ldblk_ukbb_chr1.hdf5",
  corr_1000G = "data/corr_1000G_EUR/1kg_for_prscs/ldblk_1kg_chr1.hdf5"
)
grid <- tibble(pheno = c("brca", "prca", "cad", "t2d", "t1d")) %>%
  tidyr::expand_grid(name_corr = names(ALL_CORR), chr = 1:22) %>%
  filter(!file.exists(paste0("results_prscs/sumstats_FIN/", pheno, "_", name_corr, "_chr", chr, ".rds"))) %>%
  print()
...
bigassertr::assert_dir("results_prscs/sumstats_FIN")
...
furrr::future_pwalk(grid, function(pheno, name_corr, chr) {
  ...
  saveRDS(bigreadr::fread2(res_file),
          paste0("results_prscs/sumstats_FIN/", pheno, "_", name_corr, "_chr", chr, ".rds"))
})
